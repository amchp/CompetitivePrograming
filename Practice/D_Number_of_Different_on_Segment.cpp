...
struct sstree{
  iv neutro = iv(0, vi(N, 0));
  int n, l, r, pos, val, pval;  
  vector<iv> t;
  sstree(int n): n(n), t(n<<2, iv(0LL, vi(N, 0))){}
  sstree(const vi& a): n(sz(a)), t(n<<2, iv(0LL, vi(N, 0))){ build(1, 0, n-1, a); }
  iv oper(iv& a, iv& b){
    vi frec(N, 0);
    int inv = 0;
    forn(i, N){
        frec[i] = a.se[i] || b.se[i];
        if(frec[i])inv++;
    }
    return iv(inv, frec);
  }
  ...
  iv query(int v, int tl, int tr){
    if(tl > r || tr < l) return neutro;  // estoy fuera
    if(l <= tl && tr <= r) return t[v];
    int tm = (tl + tr) >> 1;
    iv lf = query(v << 1, tl, tm);
    iv rt = query((v << 1) | 1, tm+1, tr);
    return oper(lf, rt);
  }
  ...
  int query(int ql, int qr){ l = ql, r = qr;  return query(1, 0, n-1).fi;}
};
