...
int main(){
    ...
    forn(i, n){
        fwd[i] = ii{s[i], s[i]}*pBASE[i];
        // cout << fwd[i].fi << ' ' << fwd[i].se << el;
    }
    // cout << el;
    reverse(all(s));
    forn(i, n){
        bck[i] = ii{s[i], s[i]}*pBASE[i];
        // cout << bck[i].fi << ' ' << bck[i].se << el;
    }
    // cout << el;
    stree sfwd(fwd);
    stree sbck(bck);
    while(q--){
        ...
        if(op == 2){
            ...
            ii qbck = sbck.query(rl, rr);
            if(l < rl){
                // d(rr - l);
                qfwd = qfwd * pBASE[rl - l];
            }else{
                // d(l - r);
                qbck = qbck * pBASE[l - rl];
            }
            if(qfwd == qbck){
                cout << "YES" << el;
            }else{
