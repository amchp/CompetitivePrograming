...
int main() {
    ...
    while(t--){
        ...
        forn(i, 31){
            ...
            dinic2 = Dinic(n + 2, n, n + 1);
            forn(j, m){
                dinic.addEdge(cnt[j].fi, cnt[j].se, 1);
                dinic.addEdge(cnt[j].se, cnt[j].fi, 1);
                dinic2.addEdge(cnt[j].fi, cnt[j].se, 1);
                dinic2.addEdge(cnt[j].se, cnt[j].fi, 1);
            }
            forn(j, n){
                // d(j);
                if(mrk[j] == -1)continue;
                if((mrk[j] & (1 << i))){
                    dinic.addEdge(n, j, inf);
                    dinic2.addEdge(j, n + 1, inf);
                }else{
                    dinic.addEdge(j, n + 1, inf);
                    dinic2.addEdge(n, j, inf);
                }
            }
            // fans += dinic.maxFlow() * (1 << i);
            ans1 += 1LL*dinic.maxFlow() * (1LL << i);
            ans2 += 1LL*dinic2.maxFlow() * (1LL << i);
            forn(j, n){
                if(mrk[j] != -1)continue;
                if(dinic.dis[j] != -1)ans[j] |= 1 << i;
            }
        }
        // d(fans);
        assert(ans1 == ans2);
